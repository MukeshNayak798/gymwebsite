import { NextResponse } from 'next/server';
import { sql } from '@vercel/postgres';

export async function GET() {
  try {
    const memberPercentages = await sql`
      SELECT m.id, m.name, m.photo,
             COUNT(a.id)::int as total_days,
             SUM(CASE WHEN a.status = 'Present' THEN 1 ELSE 0 END)::int as present_days
      FROM members m
      LEFT JOIN attendance a ON m.id = a.member_id
      GROUP BY m.id, m.name, m.photo
    `;

    const memberSummary = memberPercentages.rows.map((m) => ({
      id: m.id,
      name: m.name,
      photo: m.photo,
      total_days: m.total_days || 0,
      present_days: m.present_days || 0,
      percentage: (m.total_days || 0) > 0 ? Math.round(((m.present_days || 0) / m.total_days) * 100) : 100,
    }));

    const dailyStats = await sql`
      SELECT attendance_date,
             SUM(CASE WHEN status = 'Present' THEN 1 ELSE 0 END)::int as present_count,
             SUM(CASE WHEN status = 'Absent' THEN 1 ELSE 0 END)::int as absent_count
      FROM attendance
      GROUP BY attendance_date
      ORDER BY attendance_date DESC
    `;

    return NextResponse.json({ members: memberSummary, dailyStats: dailyStats.rows });
  } catch (error: unknown) {
    return NextResponse.json({ error: (error as Error).message }, { status: 500 });
  }
}
